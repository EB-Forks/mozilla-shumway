...
function isContentWindowPrivate(win) {
  console.warn("TODO:", "isContentWindowPrivate(win=" + win + ")");
  return false;
}
...
function getStartupInfo(element) {
  ...
  if (tagName === 'EMBED') {
    for (var i = 0; i < element.attributes.length; ++i) {
      let paramName1 = element.attributes[i].localName.toLowerCase();
      objectParams[paramName1] = element.attributes[i].value;
    }
  } else {
    for (var i = 0; i < element.childNodes.length; ++i) {
      ...
      let paramName2 = paramElement.getAttribute('name').toLowerCase();
      objectParams[paramName2] = paramElement.getAttribute('value');
    }
  }
  ...
  if (objectParams.base) {
    try {
      // Verifying base URL, passed in object parameters.
      // It shall be okay to ignore bad/corrupted base.
